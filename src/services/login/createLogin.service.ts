...
const createLoginService = async(dataLogin: TLoginRequest): Promise<TLoginResponse> => {

    const { email } = dataLogin
    const { password } = dataLogin

    const userExists: User | null = await userRepository.findOneBy({email})

    if(!userExists) throw new AppError('Invalid credentials', 401)
       
    const checkPassword: boolean = await bcrypt.compare(password, userExists.password)
